...
import '../widgets/prediction_chart_card.dart'; // Import widget prediksi
...
class _ActualDashboardContentState extends State<ActualDashboardContent> {
  ...
  void initState() {
    super.initState();
    _initializeDashboard();
    // Atur timer untuk refresh data tren setiap 5 menit
    _refreshTimer = Timer.periodic(const Duration(minutes: 5), (timer) {
      if (_selectedDevice != null) {
        _fetchTrendingData(
          showLoading: false,
        ); // Refresh tanpa menampilkan loading indicator
      }
    });
  }
  ...
  @override
  void dispose() {
    _refreshTimer?.cancel(); // Selalu batalkan timer untuk mencegah memory leak
    super.dispose();
  }
  ...
  Future<void> _initializeDashboard() async {
    ...
    try {
      // Ambil nama user dan daftar perangkat secara bersamaan
      final userDetailsFuture = _authService.getUserDetails();
      final devicesFuture = _deviceService.getDevices();

      final results = await Future.wait([userDetailsFuture, devicesFuture]);

      final userDetails = results[0] as Map<String, String?>;
      ...
      if (mounted) {
        ...
        if (_devices.isNotEmpty) {
          _selectedDevice = _devices.first;
          // Setelah perangkat dipilih, ambil data tren untuk perangkat tersebut
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error memuat data awal: $e'),
            backgroundColor: Colors.red,
          ),
        );
    } finally {
  ...
  Future<void> _fetchTrendingData({bool showLoading = true}) async {
    ...
    try {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error memuat data tren: $e'),
            backgroundColor: Colors.red,
          ),
        );
    } finally {
      if (mounted && showLoading)
        ...
        setState(() {}); // Refresh UI bahkan tanpa loading indicator
    }
  }
  ...
  void _onDeviceChanged(Device? device) {
    if (device != null) {
      setState(() {
        _selectedDevice = device;
        _trendingData = null; // Hapus data lama saat mengganti perangkat
      });
      _fetchTrendingData();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        int crossAxisCount = 2; // Default untuk layar sedang
        double childAspectRatio = 2; // Rasio untuk kartu summary
        if (constraints.maxWidth < 600) {
          crossAxisCount = 1; // 1 kolom di layar mobile
          childAspectRatio = 2.5;
        } else if (constraints.maxWidth > 1200) {
          crossAxisCount = 4; // 4 kolom di layar sangat lebar
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // 2. Dropdown Pemilih Perangkat
              _buildDeviceSelector(),
              const SizedBox(height: 24),

              // 3. Tampilkan loading atau konten utama
              _isLoading
                  ? const Center(
                      heightFactor: 10,
                      child: CircularProgressIndicator(),
                  ...
                  ? const Center(
                      child: Text("Silakan tambah perangkat terlebih dahulu."),
                    )
                  : Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        // 4. Kartu Data Tren
                        _buildTrendingCards(crossAxisCount, childAspectRatio),
                        const SizedBox(height: 24),

                        // 5. Kartu Grafik Prediksi
                        const Text(
                          'Prediksi Konsumsi Besok',
                          style: TextStyle(
                            fontSize: 18,
                            fontWeight: FontWeight.bold,
                          ),
                        ...
                        const SizedBox(height: 12),
                        PredictionChartCard(deviceId: _selectedDevice!.id),
                      ],
                    ),
            ],
          ),
  ...
  Widget _buildTrendingCards(int crossAxisCount, double childAspectRatio) {
